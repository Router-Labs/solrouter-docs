...
| publicKeySha256 | string | Hex sha256 of the raw 32-byte public key. `GET /tee/attestation` returns this digest as `reportDataHex` and binds it into the quote's `report_data` as `sha512("app-data:" || digest)`. |
