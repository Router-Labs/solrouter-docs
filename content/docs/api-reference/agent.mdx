...
| `model` | string | The model that runs the loop and writes the reply. `gpt-oss:20b` (default), `qwen3.8:27b` (uncensored), or `qwen3-coder:30b` (coding). A model with no configured Nosana endpoint returns 501 `nosana_not_configured`. |
...
| `encryptedPrompt` | string (JSON) | Encrypted mode. The prompt is encrypted client-side to the enclave public key and packaged as a JSON string with `ciphertext`, `nonce`, `publicKey`, `algorithm`, and `version`. The backend forwards it to the CVM without reading it. Status: Live for REST callers who send `encryptedPrompt` and for the chat app's Agent mode (the default; Extended tools uses the plaintext path). Soon for the SDK. Metered like `/tee/process`, and each call writes a Solana receipt. |
