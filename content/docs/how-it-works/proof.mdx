...
Each sealed call through `POST /tee/process` (the route the SDK and the MCP server's chat tools use) writes one receipt. Each receipt is a **Light Protocol compressed account**: about 0.000005 SOL each, roughly 400 times cheaper than a normal Solana PDA. That cost gap is why a proof per message is viable.
