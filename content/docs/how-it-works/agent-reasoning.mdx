...
The encrypted path is Live for REST callers who send `encryptedPrompt` and for the chat app's Agent mode by default, and Soon for the SDK. Inside the enclave the agent picks one of 40 decision graphs written offline and runs only that graph's tools. The chat app's Extended tools switch runs the plaintext tool loop instead.
