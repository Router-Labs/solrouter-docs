...
Rows here describe the sealed path: the TypeScript SDK with its default settings, or the MCP server's `encrypted_chat`. Amber cells mark the only places a party can read your words. Every row is Live.
...
The strip below follows one sealed request from your device to the Solana receipt. Green zones hold your words in readable form. Grey zones hold only ciphertext or a hash. The amber zone is the rented GPU machine.
