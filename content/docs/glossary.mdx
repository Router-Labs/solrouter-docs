...
**Guest mode.** Using the chat app without signing in. Guests get 5 free messages per day per network address. See [Chat app](/docs/use/chat-app).
## I
...
## S
**Sealed.** Encrypted on your device to the enclave's key (X25519 per request plus RescueCipher), opened only inside the Intel TDX enclave, and recorded with a receipt on Solana. The SDK and the MCP server's chat tools are sealed; Agent mode and its connectors are not. See [What is private here](/docs/use/what-is-private).
