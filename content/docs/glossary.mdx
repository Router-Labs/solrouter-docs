...
**API key.** A secret string that starts with `sk_solrouter_`. Send it with a request so Solrouter knows which account and plan to charge. Treat it like a password. See [Get an API key](/docs/build/api-key).
...
**Guest mode.** Using the chat app without signing in. Guest messages are sealed, and guests get 5 per day per network address. See [Chat app](/docs/use/chat-app).
## I
...
**Open-weight model.** An AI model whose files are public, so anyone can download and run it on their own hardware. This is what makes private hosting possible. Solrouter runs `gpt-oss:20b`, `qwen3.8:27b`, and `qwen3-coder:30b`. See [Models](/docs/how-it-works/models).
## P
...
**Plan.** A monthly allowance of private calls, paid in USDC on Solana: Pro \$9 for 2,000 calls, Builder \$49 for 12,500, Scale \$99 for 30,000, each per 30 days. Card payments go through MoonPay. See [Pricing](/docs/use/pricing).
## Q
...
**report_data.** A 64-byte field inside a TDX quote, at byte 568 of a version 4 quote. Solrouter asks Phala's guest agent for a quote over a hash of its public key, and the agent writes `sha512("app-data:" || that hash)` into the field. `GET /tee/attestation` hashes the X25519 key alone; per-reply quotes hash the X25519 key with the ed25519 signing key. See [Verify the enclave](/docs/verify).
...
## S
**Sealed.** Encrypted on your device to the enclave's key (X25519 per request plus RescueCipher), opened only inside the Intel TDX enclave, and recorded with a receipt on Solana. The web chat, the SDK, and the MCP server's chat tools are sealed; Agent mode and its connectors are not. See [What is private here](/docs/use/what-is-private).
...
**Solana wallet.** An app that holds your Solana keys and signs actions for you. You can sign in to Solrouter with one, with no identity check. Phantom, Solflare, or a Privy embedded wallet all work; signing in with email makes an embedded wallet for you. See [Get an API key](/docs/build/api-key).
## T
...
**USDC.** A digital dollar on Solana, meant to stay worth one US dollar. Solrouter plans and pay-per-call x402 payments are in USDC. See [Pricing](/docs/use/pricing).
## V
