...
Every priced route is charged on each call. That includes the per-wallet routes under `/agents/v1/wallets/:id`. With an API key, the price is debited from the account's USDC balance. Without a key, or when the balance is too low, the route answers `402 Payment Required` with an x402 challenge. A plan's calls do not cover these routes; they cover inference only.
...
| `POST /api/v1/x402/chat/completions` | \$0.02 |

Routes not listed carry no per-call price: the anonymity set, one-shot `execute`, wallet `audit`, sessions, attestations, and the discovery documents.
...
* **Pricing:** \$0.02 per call, settled via x402 USDC on Solana mainnet. A signed-in caller with an active plan spends one plan call instead
...
<Callout>Every x402 price comes from `ROUTE_PRICES` in the backend, the table that also feeds `/.well-known/x402`. The full list is on [Pricing](/docs/use/pricing#agents-without-an-account-x402).</Callout>
